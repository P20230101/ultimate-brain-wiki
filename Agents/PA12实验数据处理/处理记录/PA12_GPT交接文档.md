...
- 自建 VFM 与全量回归测试：阶段 I 更新后 `104/104` 通过，包含双轴默认映射、逐实验单轴映射、边界/虚场长度和阶段 2 合成单轴端到端映射测试。
...
- 阶段 A 逐帧一致性复核：679 行均通过照片唯一、时间严格递增、阶段 1 `E×逐点虚场系数`及力残差、阶段 2模型内部虚功及残差、CSV 与 JSON 面积统计一致性检查。当前逐实验 JSON 的阶段 1相对 RMSE 为 S15 `0.11199813`、S16 `0.04762204`、S17 `0.05649655`、S18 `0.01355514`；S15 超过 `0.10` 门槛，四组面积比均低于 `0.95`。S16/S18 固定窗口 Y/H 漂移大，严格留出 RMSE 分别为 Job ROI `19.05/29.02 MPa`；正式参数门槛不通过。
- 自建 VFM 阶段 B/I：S19–S22 已接入真实 Job Polygon 和逐点三角形积分；单轴机器轴映射改为逐实验显式候选。S19 `E=5389.40/Y=8.94/H=23357.94 MPa`、S20 `17600.49/41.74/18526.97 MPa`、S21 `16912.33 MPa`（Y/H 点数不足）；S22 按照片/ROI 支持的 Y→DIC y 候选重算为 `E=10535.04/Y=113.09/H=202.77 MPa`。S22 从旧映射下非正 E 改为正值，显示结果对映射敏感；新映射仍缺传感器—Job/DIC 坐标标定。全部单轴继续 `REVIEW_REQUIRED`，不得作为材料参数或训练标签。详见[阶段 B](../VFM自建/汇总/PA12自建VFM阶段B单轴Polygon复核.md)、[阶段 G](../VFM自建/汇总/PA12自建VFM阶段G S22单轴横向应变诊断.md)和[阶段 I](../VFM自建/汇总/PA12自建VFM阶段I单轴机器轴与DIC轴映射复核.md)。
...
0. 保留 STEP、`verification.json` 与实际厚度口径的证据分界；当前短 STEP 已独立复核名义几何，长文件名映射只作为血缘补证项。下一步取得实际截面/厚度或图像—CAD 配准证据，在此之前不把 CAD 名义值写成实物测量，也不解除 S15 ROI containment 门槛。
...
1. 读取本文件、机器可读状态、总目标与阶段计划、总体方法协议和阶段 A–I 报告；以机器状态中的最新阶段和曲线审计为准。
2. 自建 VFM 是计算主线；MatchID `.vfm`、Boundary/Forces 和 GUI 参数仅作格式、方向和历史审计，不作为计算依赖或停工条件。
...
6. S19–S22 只允许真实 Job Polygon 诊断，必须维持 `REVIEW_REQUIRED`。代码已将单轴机器 X/Y→DIC x/y 映射改为逐实验显式配置；S19–S21 与 S22 均是尚未独立标定的候选。正式识别前仍须核实传感器/夹具坐标、实际厚度、有效宽度、标距、积分域含义和有向外功边界。
