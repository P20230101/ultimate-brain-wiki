...
- 自建 VFM 目标测试：`15/15` 通过；当前全量测试：`94/94` 通过；包含同步、DIC、边界、检查器和自建 VFM 公式测试。
- Python 编译检查：`python -m compileall -q tools tests` 于 2026-09-25 通过。
...
- 自建 VFM 阶段 A：S15/S16/S17/S18 分别输出 284/257/12/126 帧；逐帧正式积分实际为 `pointwise_triangle`，最低面积比约为 0.895428、0.892248、0.905004、0.891147，四组均为 `SELF_VFM_REVIEW_REQUIRED`。候选值分别为 S15 `E=3536.33/Y=34.06/H=154.56 MPa`、S16 `3405.14/39.50/156.40 MPa`、S17 `E=4991.17 MPa`（Y/H 未计算）、S18 `4606.81/43.77/632.27 MPa`；这些值不是最终材料参数。
- 自建 VFM 阶段 B：S19–S22 已接入真实 Job Polygon 和逐点三角形积分。结果仍全部为 `REVIEW_REQUIRED`：S19 `E=23537.75/Y=0.83/H=5495.43 MPa`，S20 原始 `E=-197223.48 MPa` 后阻断阶段 2，S21 `E=101004.24 MPa` 后阻断阶段 2，S22 `E=2415.93/Y=23.75/H=54.59 MPa`。四组 Polygon 有效面积比均低于 0.95；单轴实际厚度、有效宽度、标距、积分域和外部虚功边界仍无独立证据，因此这些数值不是材料参数或代理模型标签。详见 [阶段 B 单轴 Polygon 复核](../VFM自建/汇总/PA12自建VFM阶段B单轴Polygon复核.md)。
...
4. 使用当前固定工作几何：中心 ROI 厚度 1 mm、外围整体厚度 3 mm、有效宽度 30 mm、标距 30 mm；`Press` 按用户确认的 N 使用。机器力按已确认方向直接作为自建 VFM 主路径的 ROI 边界合力。
5. 先复核四组自建 VFM 的 ROI 面积比、逐帧内外虚功、残差和参数稳定性；S15 ROI、S16 `3try`/`4try_step3` 及第 16 轮 GUI 数值保留为 MatchID 历史审计记录，不再作为自建主路径的计算依赖。
6. 对四组等双轴数据分别拟合，按阶段 1（固定 `ν=0.375` 识别 `E`）、阶段 2（固定 `E、ν` 识别 `Y、H`）执行。S15/S16/S17/S18 当前均为复核状态；S16 第 16 轮参数只能作为 GUI 中间候选。
7. 在解释 MatchID 的 `Y、H` 前，按上述本构定义核验项先确认 H 的模型公式和符号。

## 9. 自建 VFM 主路径；MatchID 历史审计

当前主流程使用 `VFM自建/`。它读取已完成同步的 DIC 全场—Press 合并数据，按旋转方向将机器 X 映射到 DIC `Eyy`、机器 Y 映射到 DIC `Exx`，并使用中心厚度 1 mm 和外围边界合力直接计算虚功。MatchID 文件、`.vfm` 格式、Boundary/Forces 映射和 GUI 试算保留为历史审计证据，不作为自建计算依赖。
...
- 当前阶段 A 结果：S15/S16/S17/S18 四组均为 `SELF_VFM_REVIEW_REQUIRED`；S17_XY_20 只有 4 个阶段 2 有效点，Y/H 未计算。
- 当前下一道门槛：先解释四组面积比不足并完成 ROI/虚功/参数稳定性复核，再补齐 S19–S22 单轴几何、厚度、积分域和外功边界证据；S23 缺少视觉断裂时刻的力数据，S24 是预载释放记录。

复现入口：`tools/run_pa12_self_vfm.py`；配置：`configs/pa12_self_vfm.json`；结果总表：`Agents/PA12实验数据处理/VFM自建/汇总/PA12自建VFM结果.csv`。阶段 A 输出已可复现，但四组仍是复核结果；不能绕过数据、几何、面积和稳定性门槛直接写成正式材料参数。
