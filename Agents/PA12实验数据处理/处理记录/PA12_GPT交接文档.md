# PA12 单轴/双轴拉伸：GPT → MatchID VFM 交接文档

> 下一次 GPT 必须先读本文件、`PA12实验总体方法与防跑偏协议.md`、`PA12_GPT交接状态.json`、`PA12批量处理清单.json`、`PA12数据合理性审计报告.md`、`PA12数据合理性审计结果.json`、`MatchID_VFM准备/PA12_MatchID_VFM闭环状态.md`、`MatchID_VFM准备/PA12_环境检查.md`、`MatchID_VFM准备/PA12_DIC_DAT质量审计.md`、`MatchID_VFM准备/PA12_VFM边界载荷说明.md` 和 `configs/pa12_vfm_boundary.json`。基准使用 MatchID 自带 VFM；用户确认外围机器力直接作为 ROI 边界合力，不重建四边牵引分布。
...
4. 将同一帧的 DIC 全场位移/应变和边界载荷接入 MatchID VFM。
5. 在单位、标定、几何和边界条件确认后，再识别材料参数。

当前已完成同步力值、逐帧 DAT 质量审计、同版本 MatchID 字段交叉验证和 8 组 DIC 全场—力合并，并开始 MatchID VFM GUI 试算。同步力 CSV、DIC 合并和 GUI 中间迭代都不是最终材料参数识别结果；当前 S16 的试算分别有载荷不同步和 Forces 缺失问题。

## 1.1 最新验证状态

- 最新全量单元测试：`83/83` 通过（当前交接）；包含同步、DIC、边界、检查器和自建 VFM 公式测试。
- Python 编译检查：`python -m compileall -q tools tests` 于 2026-09-24 新鲜通过。
...
- 应力—应变审计：4 组通过，5 组需要曲线/事件人工复核，S24 为预载释放记录；未对回退应变或未确认掉载做静默平滑或补点。
...
- 检查器最新状态：`test_pa12_vfm_check.py` 定向测试 `4/4` 通过；只读调用 `build_current_check(configs/pa12_rotated_batch.json)` 成功返回四组结果，并从相邻 `configs/pa12_vfm_boundary.json` 读入 S16 候选快照及 ROI 状态。当前 Markdown/CSV 已由并行流程更新，随后经只读逐字段比对与当前 builder 结果一致；本轮未调用 writer。
...
- 当前默认等双轴同步力约定为 `X=(X1_Press+X2_Press)/2`、`Y=(Y1_Press+Y2_Press)/2`。用户已确认外围力传感器值可直接作为 MatchID 自带 VFM 所需 ROI 边界合力；不据此另行构造四边牵引分布或均布应力。
...
5. 当前双轴 VFM 工作几何固定为中心 ROI 厚度 1 mm、外围整体厚度 3 mm、有效宽度 30 mm、标距 30 mm；REV B 图纸另给出中心平坦区约 28×28 mm、DAT/照片 ROI 约 27.64×28.08 mm。单轴长条的厚度、有效宽度、标距和外功边界仍须独立证据。按已确认的机器通道—DIC 方向映射，将机器力直接用于 MatchID 自带 VFM，不恢复牵引分布。
...
1. 读取本文件、批量 JSON、审计 JSON、DAT 质量审计和 MatchID 闭环状态；先按闭环状态处理阻塞项。
2. 审核已生成检查图，确认起点、峰值、终点和照片采样点没有明显错位。
3. 保留 S23 的诊断结果，若实验记录能提供真实断裂时刻，再补充独立的 `force_fracture_index`；否则不发布正式 VFM。
4. 使用当前固定工作几何：中心 ROI 厚度 1 mm、外围整体厚度 3 mm、有效宽度 30 mm、标距 30 mm；`Press` 按用户确认的 N 使用。机器力按已确认方向直接作为 MatchID 自带 VFM 所需 ROI 边界合力。
5. 优先复核 S16 `3try` 照片—力时间对应和 `4try_step3` 的 Forces 输入计数；在两项输入门槛通过前，不接受这些试算中的参数。
6. 对四组等双轴数据分别拟合，按阶段 1（固定 `ν=0.375` 识别 `E`）、阶段 2（固定 `E、ν` 识别 `Y、H`）执行。逐帧检查虚功、残差、参数边界；S16 第 16 轮参数仅保留为 GUI 中间候选。
7. 在解释 MatchID 的 `Y、H` 前，按上述本构定义核验项先确认 H 的模型公式和符号。

## 9. MatchID 不可用时的自建 VFM 路径

为保证后续仍能接入 MatchID VFM，同时避免被软件工程文件阻塞，已建立 `VFM自建/` 替代路径。它读取已完成同步的 DIC 全场—Press 合并数据，按旋转方向将机器 X 映射到 DIC `Eyy`、机器 Y 映射到 DIC `Exx`，并使用中心厚度 1 mm 和外围边界合力直接计算虚功。

- 阶段 1：固定 `ν=0.375`，由加载初期内外虚功识别 `E`。
- 阶段 2：固定 `E、ν`，由平面应力等效应力和等效塑性应变识别 `Y、H`。
- 模型比较：Linear、通用 Ludwik、通用 Swift、通用 Voce I、通用 Voce II；通用公式不等同于 MatchID 界面同名模型。
- 当前候选：S16_XY_0.2、S18_XY_2；S15_XY_0.2 需复核；S17_XY_20 因帧数不足只能完成阶段 1。
- 当前阻塞：S19–S22 单轴几何、厚度、积分域和外功边界没有独立证据；S23 缺少视觉断裂时刻的力数据；S24 是预载释放记录。

复现入口：`tools/run_pa12_self_vfm.py`；配置：`configs/pa12_self_vfm.json`；结果总表：`Agents/PA12实验数据处理/VFM自建/汇总/PA12自建VFM结果.csv`。这些结果是候选识别结果，不能绕过数据和几何门槛直接写成正式材料参数。
