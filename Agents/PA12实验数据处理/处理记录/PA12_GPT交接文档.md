...
- 自建 VFM 阶段 B/I：S19–S22 已接入真实 Job Polygon 和逐点三角形积分；单轴机器轴映射改为逐实验显式候选。S19 `E=5389.40/Y=8.94/H=23357.94 MPa`、S20 `17600.49/41.74/18526.97 MPa`、S21 `16912.33 MPa`（Y/H 点数不足）；S22 按照片/ROI 支持的 Y→DIC y 候选重算为 `E=10535.04/Y=103.58/H=238.04 MPa`。S22 从旧映射下非正 E 改为正值，显示结果对映射敏感；新映射仍缺传感器—Job/DIC 坐标标定。全部单轴继续 `REVIEW_REQUIRED`，不得作为材料参数或训练标签。详见[阶段 B](../VFM自建/汇总/PA12自建VFM阶段B单轴Polygon复核.md)、[阶段 G](../VFM自建/汇总/PA12自建VFM阶段G S22单轴横向应变诊断.md)和[阶段 I](../VFM自建/汇总/PA12自建VFM阶段I单轴机器轴与DIC轴映射复核.md)。
...
1. 读取本文件、机器可读状态、总体方法协议和阶段 A–I 报告；将 `STAGE_I_REVIEW_REQUIRED` 视为当前状态。
...
- 当前阶段 A–I：双轴完整 Job ROI 主结果、内缩域敏感性、稳定性/留出、单轴 Polygon、E–ν 剖面、S22 横向应变窗口、源 STEP ROI 尺寸与单轴显式轴映射均已记录。四组等双轴和四组单轴仍为 `REVIEW_REQUIRED`；阶段 F 未识别稳定 ν，单轴面积比未达 0.95。阶段 I 的 S22 条件候选为 `E=10535.04/Y=103.58/H=238.04 MPa`，不取代旧数据而是更新派生诊断；映射仍待设备坐标确认。S16 实物身份/亚像素配准缺失；S17 阶段 2 点数不足，S23 断裂处无力，S24 为预载释放。详见[阶段 G](../VFM自建/汇总/PA12自建VFM阶段G S22单轴横向应变诊断.md)、[阶段 H](../VFM自建/汇总/PA12自建VFM阶段H双轴ROI与厚度几何门槛.md)和[阶段 I](../VFM自建/汇总/PA12自建VFM阶段I单轴机器轴与DIC轴映射复核.md)。
