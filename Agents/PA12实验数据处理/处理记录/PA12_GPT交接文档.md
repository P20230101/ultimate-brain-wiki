...
1. 读取本文件、机器可读状态、总体方法协议和阶段 A–G 报告；将 `STAGE_G_REVIEW_REQUIRED` 视为当前状态。
...
- 当前阶段 A–H：双轴完整 Job ROI 主结果、DIC subset 内缩域独立敏感性、固定窗口/严格留出、单轴 Polygon 诊断、S19 准入、E–ν 剖面、S22 横向应变窗口及源 STEP ROI 尺寸审计均已记录；四组等双轴和四组单轴仍为 `REVIEW_REQUIRED`。阶段 F 未识别出稳定 ν；阶段 G 的表观应变比随窗口变化且单轴映射未确认，不构成独立 ν 约束；阶段 H 的 S16 ROI 只有严格居中才可容纳于 `tc1000` 物理平坦面，但实物身份和亚像素配准缺失；S17 阶段 2 点数不足，S22 在当前条件映射下阶段 1 为非正模量，S23 断裂处无力，S24 为预载释放。详见[阶段 G](../VFM自建/汇总/PA12自建VFM阶段G S22单轴横向应变诊断.md)和[阶段 H](../VFM自建/汇总/PA12自建VFM阶段H双轴ROI与厚度几何门槛.md)。
- 当前下一道门槛：建立 S16 试样—STEP 身份对应和 Job ROI 相对中心 1 mm 平坦面的亚像素注册；确认 S22 机器 Y 是否对应图像竖直 DIC `Eyy` 轴及图像旋转，再界定独立可验证的弹性窗口。补充独立 ν 约束和外功物理边界；单轴正式识别前逐试样确认方向映射并补齐几何/外功证据。完成这些门槛后才重跑正式参数候选与跨试验验证。
