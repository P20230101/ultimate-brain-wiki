...
当前已完成同步力值、逐帧 DAT 质量审计、同版本 MatchID 字段交叉验证和 8 组 DIC 全场—力合并；自建 VFM 阶段 A–I 已形成双轴主结果/敏感性分析、单轴条件诊断、残差与稳定性复核。历史 MatchID GUI 试算仅作审计证据：S16 的试算分别有载荷不同步和 Forces 缺失问题。同步力 CSV、DIC 合并、自建 VFM 候选和 GUI 中间迭代都不是最终材料参数识别结果。
...
- 跨实验留出：S15–S18 完整 Job ROI 共 12 个有向配对，均为 `DIAGNOSTIC_ONLY`；加载速率不同且没有同速率重复样本，不能据此证明重复性或正式跨实验泛化。阶段 2相对误差按生产 runner 的最大拟合等效应力归一化；Markdown 与 CSV 由同一命令同步生成。
- 双轴 ROI 双口径复现：完整 Job ROI 继续作为主结果；DIC subset 内缩域由独立 runner 输出到 `VFM自建/敏感性分析/`。2026-09-26 复跑的 4 组帧数和 E/Y/H 与阶段 D 表一致；S17 仍因塑性点数不足没有 Y/H，S18 的 H 随域从 `632.27` 变为 `331.91 MPa`。逐实验 JSON 已刷新为阶段 I 的显式机器轴—DIC 映射记录；主结果目录和阶段 D 比较表未被覆盖。
...
### 1.2 曲线事件与自建 VFM 对照（只读，2026-09-26）

- S15：曲线 CSV 与自建 VFM 均为 `000001–000284.jpg` 的 284 帧。X 峰值位于末帧 `000284.jpg`（`53.606 MPa`），Y 峰值位于 `000283.jpg`（`53.461 MPa`），到末帧仅降至峰值的 `0.999688`。`000285.jpg` 目视已断裂，但同名 DAT 只有约 5,277 点，前序完整场约 98,000 点，因此断裂帧不能进入 DIC/VFM；没有可用于识别的峰后断裂段。阶段 2 有 258 个候选帧（`000005–000284`），虚功残差 RMSE 为 X=`275.985 N`、Y=`276.868 N`；末帧残差为 X=`218.360 N`、Y=`217.701 N`。这组力和 DIC 帧同步，但残差不等于闭合通过。
- S22：曲线峰值在 `001120.jpg`（应力=`50.3433 MPa`、应变=`0.744203`、Y 力=`1510.3 N`），末端 `001784.jpg` 为 `10.4433 MPa`、`1.203773`、`313.3 N`，峰后比例=`0.207442`。已查看峰值和末端照片；`001784.jpg` 仍可见连续试样，不能将该力降自动命名为断裂。阶段 2 仅有 125 帧，范围 `000032–001024.jpg`，在曲线峰值之前结束；Y 残差 RMSE=`163.808 N`、最大绝对残差=`772.575 N`。因此阶段 2 没有覆盖峰值或峰后段。另，曲线配置使用 `30 mm×1 mm` 名义截面及 `30 mm` 标距，而 Job ROI 尺寸约 `10.166626×44.333156 mm`；单轴实物截面/标距未确认前，不能把两种应力口径直接等同。
- S23：`001545.jpg`、`001546.jpg` 仍连续，`001547.jpg` 首次目视断开；同步力数据最后支持 `001528.jpg`（Y=`1259.37 N`，曲线应力=`41.979 MPa`、应变=`1.02067`）。断裂照片无对应力值，不能补造断裂应力；S23 当前不具备自建 VFM 输出。
- S22 的结果 JSON 阶段 2点数、逐帧 CSV 非空阶段 2记录数均为 125，首末帧均为 `000032–001024.jpg`。`001120` 及之后的阶段 2列为空是因为这些帧不在阶段 2候选窗口，不是旧 CSV 或生成失败。

### 1.3 单轴固定窗口和严格留出诊断（只读）

四窗口审计输入为完整 Job Polygon 自建 VFM CSV；窗口间拟合的 Y/H 只作区间敏感性诊断。严格留出按时间顺序 75/25 分割。S19–S21 使用首轮脚本；S22 后续已按阶段 I 的逐实验机器轴—DIC 映射完成独立计算，不能将它描述为未运行，也不能把该单点诊断当成验证通过。

| 实验 | 四窗口 Y 范围（MPa） | 四窗口 H 范围（MPa） | 训练段 Y/H（MPa） | 留出 RMSE（MPa） |
| --- | ---: | ---: | ---: | ---: |
| S19_X_0.2 | −14.65–103.88 | −4039.84–28887.28 | −17.59 / 40772.50 | 145.80 |
| S20_X_2 | 35.77–150.09 | −11742.47–21579.65 | 37.32 / 20912.54 | 71.79 |
| S21_X_20 | 16.10–111.01 | −10668.79–22637.59 | 28.01 / 16024.82 | 114.61 |
| S22_Y_0.2 | 96.59–249.20 | −320.99–629.13 | 113.09 / 202.77 | 26.11† |

窗口间 H 大幅漂移并变号。后段窗口没有弹性点，不能独立重识别 E。S22 的训练段为 167 帧、留出段为 56 帧；训练段含 3 个弹性观察点和 125 个峰前塑性拟合点，留出段只有 1 个有效峰前塑性点。该点残差/RMSE 为 `26.114771 MPa`，状态为 `INSUFFICIENT_TEST_POINTS`；表中 S22 的 E/Y/H 是训练段条件参数，单点 RMSE 不构成泛化验证。S19–S22 均保持 `REVIEW_REQUIRED`。本节只整合既有报告、CSV/JSON 和照片证据，没有重写曲线或 VFM 计算输出。
...
1. S15/S22/S23 曲线事件及自建 VFM 对照已完成只读复核，详情见第 1.2 节；事件审核仍未通过自动门槛，不能据此补造断裂力或升级曲线/参数状态。
...
6. 论文中的 `E=1800 MPa`、`ν=0.375`、初始屈服强度 `21 MPa`、硬化模量 `180 MPa` 是试样优化阶段的 Abaqus 工作参数，不是本实验已经识别出的材料参数。式 (2.19) 的负斜率与正 H 需向原始 Abaqus 塑性卡/作者核实；不能据此推断自建 VFM 或 MatchID 的 H 符号定义。
...
8. 已完成 S15/S22/S23 曲线、照片、力和可用 DIC/VFM 帧的逐项对照，并对 S19–S22 执行四窗口稳定性诊断及 75/25 严格留出；S22 使用映射感知实现，但仅有 1 个有效留出塑性点，故状态为 `INSUFFICIENT_TEST_POINTS`，不能称为验证通过。

## 9. 自建 VFM 主路径；MatchID 历史审计

当前主流程使用 `VFM自建/`。双轴完整 Job ROI 为主分析域，DIC subset 内缩有效域另存并仅作敏感性分析；两套结果不得混合。对已确认的双轴十字试样，旋转方向映射为机器 X→DIC `Eyy`、机器 Y→DIC `Exx`；中心 ROI 使用厚度 1 mm，外围整体 3 mm 单独记录，外围机器力按用户确认作为 ROI 边界合力。该映射尚未由 S19–S22 单轴装夹独立确认，当前单轴输出仅为方向条件诊断。MatchID 文件、`.vfm` 格式、Boundary/Forces 映射和 GUI 试算保留为历史审计证据，不作为自建计算依赖。即使 MatchID VFM 不可用，也继续由自建流程导出内外虚功和参数候选。
...
- 当前阶段 A–I：双轴完整 Job ROI 主结果、内缩域敏感性、稳定性/留出、单轴 Polygon、E–ν 剖面、S22 横向应变窗口、源 STEP ROI 尺寸与单轴显式轴映射均已记录。四组等双轴和四组单轴仍为 `REVIEW_REQUIRED`；阶段 F 未识别稳定 ν，单轴面积比未达 0.95。S22 阶段 1/2使用同一显式轴映射，完整 Job ROI 条件候选为 `E=10535.04/Y=113.09/H=202.77 MPa`；映射感知 75/25 留出仅有 1 个有效塑性点，状态 `INSUFFICIENT_TEST_POINTS`；阶段 2机器 Y 内部虚功残差 RMSE=`163.81 N`、最大绝对值=`772.57 N`，映射仍待设备坐标确认。S16 实物身份/亚像素配准缺失；S17 阶段 2点数不足，S23 断裂处无力，S24 为预载释放。详见[阶段 G](../VFM自建/汇总/PA12自建VFM阶段G S22单轴横向应变诊断.md)、[阶段 H](../VFM自建/汇总/PA12自建VFM阶段H双轴ROI与厚度几何门槛.md)和[阶段 I](../VFM自建/汇总/PA12自建VFM阶段I单轴机器轴与DIC轴映射复核.md)。
- 当前计算状态：S15/S22/S23 曲线事件复核、S19–S22 四窗口和 75/25 严格留出诊断已完成；S22 留出仅 1 个有效塑性点，未构成独立验证，以上诊断均未解除物理门槛。下一步取得 S16 试样—STEP 对应和 ROI 亚像素配准，确认 S19–S22 传感器/夹具—Job/DIC 坐标、单轴实测厚度/有效宽度/标距、积分域与有向外功边界，并补独立 ν 和弹性区证据；证据满足后再按冻结映射重算和评估 E/J2。正式参数门槛通过前不发布材料参数。

跨实验诊断入口：`tools/audit_pa12_self_vfm_cross_experiment.py`；报告：[PA12等双轴跨实验留出审计](../VFM自建/汇总/PA12等双轴跨实验留出审计.md)，机器明细：`Agents/PA12实验数据处理/VFM自建/汇总/PA12等双轴跨实验留出审计.csv`。只比较 S15–S18 等双轴完整 Job ROI，共 12 个有向留出配对；跨速率迁移不等于材料重复性，S17 阶段 2候选点仅 4 个而未拟合。阶段 2质量误差按生产 runner 的最大拟合等效应力 `max(σ)` 归一化；E 迁移相对 RMSE 用匹配弹性观察点的外力 RMS 归一化。使用 `--markdown-output` 与 CSV 一次生成两种报告。
