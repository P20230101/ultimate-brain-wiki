...
| S22_Y_0.2 | X→x；Y→y | 10535.04 | 113.09 | 202.77 | 0.826930 | REVIEW_REQUIRED |

`E` 使用工作假设 `ν=0.375`；Y/H 来自当前约化阶段 2，不是 MatchID 材料识别结果。S22 阶段 2的塑性应变和内部虚功现按同一机器轴—DIC映射计算：机器 Y 取 `Eyy`、虚场长度取 `Ly`。完整 Job ROI 重算得到 223 帧、`E=10535.04 MPa`、`Y=113.09 MPa`、`H=202.77 MPa`；阶段 1/2拟合指标通过各自数值阈值，但阶段 2机器 Y 内部虚功残差 RMSE 为 `163.81 N`、最大绝对值 `772.57 N`。拟合阈值通过不等于虚功闭合或参数验证。S22 最低积分面积比为 `0.826930`，单轴几何与外功门槛仍未通过。S19–S22 的候选映射也未由设备坐标标定确认，因此所有数值只作条件诊断。

## S22 机器轴映射感知的 75/25 留出

留出脚本读取该组结果 JSON 的机器轴—DIC 映射，并复用自建 VFM 配置中的弹性阈值、最小力、泊松比、塑性阈值和 ROI 长度筛选。按既定时间顺序前 75% 训练、后 25% 留出：训练/留出分别为 167/56 帧；训练段有 3 个弹性方向观察点，条件 `E=10535.04 MPa`，峰前塑性拟合 125 点，条件 `Y=113.09 MPa`、`H=202.77 MPa`。留出段只有 1 个满足峰前与塑性阈值的点，单点残差为 `26.11 MPa`，状态为 `INSUFFICIENT_TEST_POINTS`，不构成留出验证通过。保留原 75/25 划分，不为增加合格点数事后移动切分位置。

复现：

```powershell
python tools/audit_pa12_self_vfm_strict_holdout.py "Agents/PA12实验数据处理/VFM自建/实验结果/S22_Y_0.2/S22_Y_0.2_内外虚功.csv" --config configs/pa12_self_vfm.json
```
...
- 全量测试：`104 passed`；定向 `tests.test_pa12_self_vfm`：`26 passed`，覆盖单轴阶段 2塑性应变及虚功映射。
- `compileall`：通过。
- S22 严格留出按 JSON 中的显式轴映射计算；单点留出自动记为 `INSUFFICIENT_TEST_POINTS`，不作为独立验证通过。
